...
def test_double_ron_sticks_atamahane():
    # 构造双响：座位0立直（局始1棒+宣言1棒），座位1放铳给座位0和座位2
    # 头和（座位0）得4000+2000棒=6000，次和（座位2）得4000
    k = [
        [0, 0, 1], [25000] * 4, [11], [],
        ...
        [], [19],
        ["和了",
         [6000, -8000, 4000, 0], [0, 1, 0, "30符1飜1000点", "立直(1飜)"],
         [0, -8000, 4000, 0], [2, 1, 2, "30符1飜1000点", "役牌 白(1飜)"]],
    ]
    s = analyze_kyoku(k)
    assert len(s["agari"]) == 2
    # 头和(座位0)：6000 - 2000(总棒) = 4000
    assert s["agari"][0]["score"] == 4000
    # 次和(座位2)：4000 - 0 = 4000
    assert s["agari"][1]["score"] == 4000
    assert s["deltas"] == [6000, -16000, 8000, 0]